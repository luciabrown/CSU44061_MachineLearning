...
df =
pd.read_csv("Week2Assignment/week2.php.csv",header=None,comment="#",sep=",",skipinitialspace=T
rue)
#print(df.head())
X1=df.iloc[:,0] # Col1
X2=df.iloc[:,1] # Col2
...
plt.scatter(X1[y==1],X2[y==1],marker="+",color="lime",label="Target = +1")
plt.scatter(X1[y==-1],X2[y==-1],marker="o",color="blue",label="Target = -1")
plt.scatter(X1_test[(y_test == y_pred) & (y_test == 1)], X2_test[(y_test == y_pred) & (y_test 
== 1)], facecolors='none', edgecolors='red', s=100, label="Classifier Predicted +1")
plt.scatter(X1_test[(y_test == y_pred) & (y_test == -1)], X2_test[(y_test == y_pred) & (y_test 
== -1)], facecolors='none', edgecolors='darkred', s=100, label="Classifier Predicted -1")
